			 ...
			 int port, int win_id, struct rkvop2_platdata *platdata)
{
	...
	if (is_cluster) {
		cluster = priv->regs + VOP2_CLUSTER_OFFSET(layer);

		switch (fb_bits_per_pixel) {
		case 16:
			rgb_mode = RGB565;
			writel(V_RGB565_VIRWIDTH(hactive), &cluster->win0_vir);
			break;
		case 24:
			rgb_mode = RGB888;
			writel(V_RGB888_VIRWIDTH(hactive), &cluster->win0_vir);
			break;
		case 32:
		default:
			rgb_mode = ARGB8888;
			writel(V_ARGB888_VIRWIDTH(hactive), &cluster->win0_vir);
			break;
		}

		writel(fbbase, &cluster->win0_mst_yrgb);
		writel(V_ACT_WIDTH(hactive - 1) | V_ACT_HEIGHT(vactive - 1),
		       &cluster->win0_act_info);
		writel(V_DSP_WIDTH(hactive - 1) | V_DSP_HEIGHT(vactive - 1),
		       &cluster->win0_dsp_info);
		writel(0, &cluster->win0_dsp_st);
		writel(0x10001000, &cluster->win0_scl_factor_yrgb);
		writel(V_ESMART_REGION0_DATA_FMT(rgb_mode) |
		       M_ESMART_REGION0_MST_EN, &cluster->win0_ctrl0);
		writel(BIT(0), priv->regs + VOP2_CLUSTER_OFFSET(layer) +
		...
		writel(0x10001000, &esmart->esmart_region0_scl_factor_yrgb);
	...
	/* Set the window to the destination video port */
	if (is_cluster)
		reg = V_CLUSTER_SEL_PORT(layer, port);
	else
